...
export default function AuthorizationCenter() {
  ...
  const [authorizations, setAuthorizations] = useState([
    { id: 1, name: 'Google OAuth 2.0', status: '活跃', date: '2024-01-10' },
    { id: 2, name: 'Stripe Payment API', status: '活跃', date: '2024-01-12' },
    { id: 3, name: 'AWS S3 存储', status: '待审核', date: '2024-01-15' },
    { id: 4, name: 'SendGrid Email', status: '活跃', date: '2024-01-08' },
    { id: 5, name: 'GitHub OAuth', status: '已停用', date: '2024-01-14' },
  ]);
      ...
      try {
        ...
        if (Array.isArray(items)) {
          const mapped = items.map((it: any, idx: number) => ({
            ...
            date: it.date ?? it.created_at ?? new Date().toISOString().split('T')[0],
            ...it,
          }));
  ...
  const handleEdit = (auth: any) => {
    ...
    setFormData({
      ...
      budget_limit: auth.budget_limit ?? '',
      budget_usage: auth.budget_usage ?? '',
      ...
      auto_approve_limit: auth.auto_approve_limit ?? '',
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-white/10 bg-[#050505] p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: '总授权数', value: authorizations.length.toString(), color: 'bg-blue-50 dark:bg-blue-900/10' },
          { label: '活跃授权', value: authorizations.filter(a => a.status === '活跃').length.toString(), color: 'bg-green-50 dark:bg-green-900/10' },
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#0B1220] border-b border-white/10">
                <th className="text-left py-4 px-6 text-slate-400 font-semibold text-sm">授权名称</th>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 overflow-auto">
          <div className="bg-[#0B1220] border border-white/10 rounded-2xl p-8 w-full max-w-2xl mx-4 shadow-2xl max-h-[85vh] overflow-y-auto modal-scrollbar">
            <h2 className="text-2xl font-bold text-white mb-6">
              {editingAuth ? '编辑授权' : '添加新授权'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">

              {/* 基础身份信息 */}
              <div className="pt-4 border-t border-white/10">
                <h4 className="text-sm font-semibold text-white">基础身份信息</h4>
                <p className="text-xs text-slate-400 mt-1">请输入智能体名称与描述；状态用于启用/冻结/过期等。</p>
              </div>
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">授权名称</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
